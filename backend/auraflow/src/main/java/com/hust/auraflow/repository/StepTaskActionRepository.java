...
public interface StepTaskActionRepository extends JpaRepository<StepTaskAction, Long> {
    ...
    List<StepTaskAction> findByTenantIdAndUserIdOrderByCreatedAtDesc(@Param("tenantId") Long tenantId,
            @Param("userId") Long userId);

    /**
     * Delete all step task actions for a given task.
     */
    @Modifying
    @Query("DELETE FROM StepTaskAction sta WHERE sta.task.id = :taskId")
